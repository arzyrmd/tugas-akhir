{{-- resources/views/invoices/custom-full.blade.php --}}
<!DOCTYPE html>
<html lang="id">
...
<head>
    ...
    <title>Invoice Pembayaran Pelunasan - {{ $customRequest->full_payment_code }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
            ...
            <div class="header-content">
                ...
                <div class="invoice-info">
                    ...
                    <div class="invoice-meta">
                        <strong>No. Invoice:</strong> {{ $customRequest->full_payment_code }}<br>
                        <strong>Tanggal:</strong>
                        {{ $customRequest->full_payment_date ? $customRequest->full_payment_date->format('d M Y') : now()->format('d M Y') }}<br>
                        <strong>Jatuh Tempo:</strong>
                        {{ $customRequest->full_payment_date ? $customRequest->full_payment_date->format('d M Y') : now()->format('d M Y') }}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="content-area">
            ...
            <div class="customer-shipping">
                ...
                @if ($customRequest->shipment)
                    <div class="shipping-info">
                        ...
                        <div class="info-box">
                            <div class="info-title">{{ $customRequest->shipment->full_name }}</div>
                            <p> {{ $customRequest->shipment->address }}</p>
                            <p> {{ $customRequest->shipment->city->name ?? '' }},
                                {{ $customRequest->shipment->province->name ?? '' }}</p>
                            <p> {{ $customRequest->shipment->postal_code }}</p>
                            <p> Telp: {{ $customRequest->shipment->phone }}</p>
                        </div>
                    </div>
                @endif
            </div>
            ...
            <div class="section">
                ...
                <div class="product-detail-box">
                    ...
                    @endif
                    <p><strong>Status:</strong> <span class="status-badge status-success"> Selesai Dikerjakan</span>
                    </p>
                    <p><strong> Tanggal Permintaan:</strong> {{ $customRequest->created_at->format('d M Y') }}</p>
                    @if ($customRequest->approved_at)
                        <p><strong> Tanggal Persetujuan:</strong> {{ $customRequest->approved_at->format('d M Y') }}
                        </p>
                    @endif
                </div>
            </div>
            ...
            <div class="section">
                ...
                <table class="payment-details">
                    ...
                    <tbody>
                        <tr>
                            <td><strong>Harga Produk Kustom</strong><br><small>{{ $customRequest->title }}</small>
                            </td>
                            <td>{{ $customRequest->approved_at ? $customRequest->approved_at->format('d M Y') : '-' }}
                            </td>
                        ...
                        <tr>
                            <td><strong> Pembayaran DP (Uang Muka)</strong><br><small>Kode:
                                    {{ $customRequest->dp_payment_code }}</small></td>
                            <td>{{ $customRequest->dp_payment_date ? $customRequest->dp_payment_date->format('d M Y') : '-' }}
                            </td>
                        ...
                        @if ($customRequest->shipment)
                            <tr>
                                <td><strong> Biaya Pengiriman</strong><br><small>Ke:
                                        {{ $customRequest->shipment->city->name ?? '' }},
                                        {{ $customRequest->shipment->province->name ?? '' }}</small></td>
                                <td>{{ $customRequest->full_payment_date ? $customRequest->full_payment_date->format('d M Y') : now()->format('d M Y') }}
                                </td>
                                <td><span class="status-badge status-success"> Lunas</span></td>
                                <td class="amount">Rp
                                    {{ number_format($customRequest->shipment->shipping_cost, 0, ',', '.') }}</td>
                            </tr>
                        @endif

                        <tr>
                            <td><strong>Sisa Pembayaran (Pelunasan)</strong><br><small>Kode:
                                    {{ $customRequest->full_payment_code }}</small></td>
                            <td>{{ $customRequest->full_payment_date ? $customRequest->full_payment_date->format('d M Y') : now()->format('d M Y') }}
                            </td>
                            <td><span class="status-badge status-success"> Lunas</span></td>
                            <td class="amount">Rp {{ number_format($customRequest->remaining_payment, 0, ',', '.') }}
                            </td>
                        </tr>

                        <tr class="total-row">
                            <td colspan="3"><strong>💸 TOTAL YANG DIBAYAR HARI INI</strong></td>
            ...
            <div class="notes">
                ...
                <ul>
                    <li> Invoice ini merupakan bukti sah pembayaran pelunasan produk kustom</li>
                    <li>Produk akan segera diproses untuk pengiriman dalam 1-2 hari kerja</li>
